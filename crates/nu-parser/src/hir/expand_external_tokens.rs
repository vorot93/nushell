use crate::hir::syntax_shape::expression::expand_file_path;
    ...
    fn expand<'a, 'b>(&self, token_nodes: &'b mut TokensIterator<'a>) -> ExternalTokensSyntax {
        ...
        loop {
            match token_nodes.expand_syntax(ExternalExpressionShape) {
                ...
                Ok(span) => {
                    let string =
                        expand_file_path(span.slice(&token_nodes.source()), token_nodes.context());
                    out.push(string.display().to_string().spanned(span));
                }
            }
        }
