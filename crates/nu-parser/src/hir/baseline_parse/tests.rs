use crate::commands::classified::external::{ExternalArgs, ExternalCommand};
...
fn test_parse_command() {
    parse_tokens(
        ...
        "^echo",
        vec![b::external_command("echo")],
        |tokens| {
            ClassifiedCommand::External(ExternalCommand::new(
                "echo".to_string(),
                Tag {
                    span: tokens[0].span(),
                    anchor: None,
                },
                ExternalArgs::new(vec![], Span::unknown()),
            ))
        },
    );
